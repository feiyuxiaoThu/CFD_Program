...
for i=1:num_x
W(i,1)=2*S(i,2)/(dy*dy);
W(i,num_y)=(2*S(i,num_y-1)-2*U*D)/(dy*dy);   %会改变
end
...
for j=2:num_y-1
    W(1,j)=2*(S(2,j)-S(1,j))/(dx*dx)+(S(1,j+1)-2*S(1,j)+S(1,j-1))/(dy*dy);  %会改变
end
...
for i=2:num_x-1
    for j=2:num_y-1
        ...
        S(i,j)=0.25*(SS(i+1,j)+SS(i-1,j)+SS(i,j+1)+SS(i,j-1))-dx*dy*W(i,j);
