...
while 1
   ...
   dt=CFL*dx/max(Lt);
    ...
    for i=2:num-1
        ...
        F1=0.5*(ro(i+1)*u(i+1)+ro(i)*u(i))-0.5*LL1 *(ro(i+1)-ro(i));
        F2=0.5*(ro(i)*u(i)+ro(i-1)*u(i-1))-0.5*LL2 *(ro(i)-ro(i-1));
        rro(i)=ro(i)-(dt/dx)*(F1-F2);
        
        FF1=0.5*(ro(i+1)*u(i+1)^2+p(i+1)+ro(i)*u(i)^2+p(i))-0.5*LL1  *(ro(i+1)*u(i+1)-ro(i)*u(i));
        FF2=0.5*(ro(i)*u(i)^2+p(i)+ro(i-1)*u(i-1)^2+p(i-1))-0.5*LL2 *(ro(i)*u(i)-ro(i-1)*u(i-1));
        uu(i)=(1/rro(i))*(ro(i)*u(i)-(dt/dx) *(FF1-FF2));
        
         FFF1=0.5*(u(i+1)*e(i+1)*ro(i+1)+p(i+1)*u(i+1)+u(i)*e(i)*ro(i)+p(i)*u(i))-0.5*LL1  *(ro(i+1)*e(i+1)-ro(i)*e(i));
        FFF2=0.5*(u(i)*e(i)*ro(i)+p(i)*u(i)+u(i-1)*e(i-1)*ro(i-1)+p(i-1)*u(i-1))-0.5*LL2 *(ro(i)*e(i)-ro(i-1)*e(i-1));
        ee(i)=(e(i)*ro(i)-(dt/dx) *(FFF1-FFF2))/rro(i);
